...
BarWidget {
  ...
  function selectDate(key) {
    if (panelLoader.item && panelLoader.item.selectDayKey)
      panelLoader.item.selectDayKey(key)
  }
  ...
  Loader {
    ...
    source: Qt.resolvedUrl("Panel.qml") + "?rev=editor-caret-1"
    visible: false
    onLoaded: {
      root.injectPanel()
    ...
    onStatusChanged: {
      if (status === Loader.Error)
        console.warn("austraz.clock Panel.qml failed to load")
    }
  }

  // Calendar hotkey and `omarchy-shell omarchy.clock *` still use the
  // built-in id. Register both so the clone owns the popup after the
  // stock plugin is disabled.
  IpcHandler {
    ...
    function selectDate(key: string): void { root.selectDate(key) }
  }

  IpcHandler {
    target: "austraz.clock"
